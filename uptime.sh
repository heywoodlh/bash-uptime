...
netcat_check() {
    netcat_services=$(echo "${yaml}" | grep 'netcat_services' | cut -d '(' -f 2 | sed 's/["()]//g')
    netcat_options=$(echo "${yaml}" | grep 'netcat_options' | cut -d '(' -f 2 | sed 's/["()]//g')
    netcat_silent=$(echo "${yaml}" | grep 'netcat_silent' | cut -d '(' -f 2 | sed 's/["()]//g')

    if [[ -n ${netcat_services} ]]
    then
        for url in ${netcat_services}
        do
            hostname="$(echo "${url}" | cut -d':' -f1)"
            port="$(echo "${url}" | cut -d':' -f2)"
            sanitized_url="${hostname}_${port}"
            outfile="${STATUS_DIR}/${sanitized_url}_netcat"
            # If netcat silent config set
            if [[ ${netcat_silent} == "true" ]]
            then
                if nc ${netcat_options} "${hostname}" "${port}" &> /dev/null
                then
                    # Log status to status file
                    if [[ ${SILENCE_DUPES} == 'true' ]]
                    then
                        # Check if status file already contains status
                        if ! grep -q ': UP service' "${outfile}" 2>/dev/null
                        then
                            echo "$(date --iso-860=seconds): UP service ${url} responded to netcat" | tee ${outfile}
                        fi
                    else
                        echo "$(date --iso-860=seconds): UP service ${url} responded to netcat"
                    fi
                else
                    # Log status to status file
                    if [[ ${SILENCE_DUPES} == 'true' ]]
                    then
                        # Check if status file already contains status
                        if ! grep -q ': DOWN service' "${outfile}" 2>/dev/null
                        then
                            ## Here
                            echo "$(date --iso-860=seconds): DOWN service ${url} unresponsive to netcat" | tee ${outfile}
                        fi
                    else
                        echo "$(date --iso-860=seconds): DOWN service ${url} unresponsive to netcat"
                    fi
                fi
            # If netcat silent config not set
            else
                if nc ${netcat_options} "${hostname}" "${port}"
                then
                    # Log status to status file
                    if [[ ${SILENCE_DUPES} == 'true' ]]
                    then
